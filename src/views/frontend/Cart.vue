    ...
    <div id="content" v-if="cart.total!==0">
      ...
      <div class="cart-products">
        <div class="container">
          <div class="row">
            <div
              class="col-12 mb-5 border-bottom my-2 d-lg-none"
              ...
              <div class="container">
                <div class="row">
                  ...
                  <div class="col-8 d-flex text-right justify-content-end align-items-center">
                    <a
                      href="#"
                      class="btn btn-outline-danger btn-lg d-flex"
                ...
                <div class="row">
                  <div class="col-7 pr-0">
                    <a href="#" class="d-block text-black">
                      <div class="product-text">{{ item.product.title}}</div>
                    </a>
                  </div>
                  <div class="col-5 text-right font-weight-bold">
                    <div>X{{ item.qty}}</div>
                    <div
                      :class="{'line-through' : item.total!==item.final_total}"
                    >{{item.total | currency}}</div>
                  </div>
                </div>
                <div class="row mb-3">
                  <div class="col-7 pr-0 text-success" v-if="item.coupon">已套用優惠券</div>
                  <div
                    class="col-5 text-right font-weight-bold text-success"
                    v-if="item.coupon"
                  >{{item.final_total | currency}}</div>
                </div>
              </div>
            </div>
                      ...
                      <td scope="col">
                        <div class="row">
                          <div
                            class="col-12"
                            :class="{'line-through' : item.total!==item.final_total}"
                          >{{item.product.price | currency}}</div>
                          <div
                            class="col-12 text-success"
                            v-if="item.total!==item.final_total"
                          >{{`${item.final_total/item.qty}` | currency}}</div>
                        </div>
                      </td>
            ...
            <div class="col-12 col-lg-6 refresh">
              <div class="container mb-4">
                <div class="row">
                  <div class="col-12 col-md-6 mb-3">
                    <a
                      class="btn btn-primary btn-sm btn-block btn-set text-white"
                  ...
                  <div class="col-12 col-md-6">
                    <router-link
                      to="/shoppage"
                      class="btn btn-outline-primary btn-sm btn-block btn-set"
                    >繼續購物</router-link>
                  </div>
                </div>
              </div>
              <div class="row coupon">
                ...
                <div class="col-12 col-lg-4">
                  <button class="btn btn-primary btn-sm button-height" @click.prevent="addCouponCode">套用優惠券</button>
                </div>
              </div>
            </div>
              ...
              <div class="container">
                <div class="row justify-content-end">
                  <div class="col-12 col-lg-8">
                    ...
                    <div class="row mb-4">
                      <div
                        class="col-6 text-success subtitle"
                        v-if="cart.final_total !== cart.total"
                      >折後訂單金額</div>
                      <div
                        class="col-6 text-right price text-success"
                        v-if="cart.final_total !== cart.total"
                      >{{ cart.final_total | priceFloor}}</div>
                    </div>
                    <div class="row mb-2">
                      <div class="container">
                        <div class="col-12">
...
#content .coupon-gocheck .button-height{
  height: 38px;
}
