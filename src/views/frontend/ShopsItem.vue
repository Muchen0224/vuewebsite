    ...
    <div id="content">
      <div class="route">
        <div class="container">
          ...
          <router-link to="/shoppage">商品列表</router-link>
          <span class="routeDivider">/</span>
          ...
          <div class="description">
            <ul class="nav nav-tabs pb-2">
              <li class="nav-item">
                <a
                  class="nav-link"
                  :class="{'active': tabActive==='tab-1'}"
                  href="#"
                  @click.prevent="tabActive='tab-1'"
                >產品內容</a>
              </li>
              <li class="nav-item">
                <a
                  class="nav-link"
                  :class="{'active': tabActive==='tab-2'}"
                  href="#"
                  @click.prevent="tabActive='tab-2'"
                >商品資訊</a>
              </li>
              <li class="nav-item">
                <a
                  class="nav-link"
                  :class="{'active': tabActive==='tab-3'}"
                  href="#"
                  @click.prevent="tabActive='tab-3'"
                >常見問題</a>
              </li>
            </ul>
            <div class="tab-content position-relative">
              <div class="nav-content" :class="{'active': tabActive==='tab-1'}">{{product.content}}</div>
              ...
              <div class="nav-question" :class="{'active': tabActive==='tab-3'}">
                ...
                <ul>
                  <li class="question">冷凍常溫可以常溫可以合併購買嗎？</li>
                </ul>
                ...
                <ul>
                  <li class="question">商品多久會到呢？</li>
                </ul>
                <p>
                  宅配 約 1～3 天；超商取件 約 2～4 天(自出貨通知時間起算)
                  <br />【 貓咪星球出貨時間與異動 】
                  ...
                  晚配為下午的14點～18點
                  不指定時段
                  <br />※以上時段為「建議到貨時間」，僅供毛拔麻參考，實際配送狀況依貨運大哥的路線安排為準，亦無法指定時間點喔！（例如 09:00整、14:30前皆為無效的指定喲）
                ...
                <ul>
                  <li class="question">冷凍生食可以超商取貨嗎？</li>
                </ul>
                <p>
                  冷凍商品可以使用全家超商取貨喔！
                  生食全程低溫冷凍配送至門市，並存放在門市冷凍庫，毛拔麻可以放心喲！
                  ※由於每間門市的冷凍庫庫存狀況不一，若遇到門市庫存空間不足的情況，可能會無法順利下單，可以改其他全家門市取件喔！
                </p>
              </div>
            </div>
          </div>
      ...
      <div class="sameClassItem mt-5">
        <div class="title">相似的產品</div>
        <div class="container">
          <swiper class="swiper" :options="swiperOption">
            <swiper-slide v-for="item in sameClassProduct" :key="item.id">
              <a href="#" @click.prevent="changeProductPage(item.id)" class="d-block">
...
#content .input-wrap .add,
#content .input-wrap .reduce {
  height: 38px;
}
...
export default {
  ...
  methods: {
    ...
    changeWish (product) {
      const vm = this
      vm.wishId = vm.wished.map(item => {
        return item.id
      })
      const index = vm.wishId.findIndex(id => {
