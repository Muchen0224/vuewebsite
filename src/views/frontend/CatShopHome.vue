...
#content .new-sale .swiper-slide{
  overflow: hidden;
}

#content .new-sale .swiper-slide img{
  transform: scale(1,1);
  transition: all 0.5s ease-out;
}

#content .new-sale .swiper-slide img:hover{
...
@media (min-width: 768px) {
  #content .new-sale .carousel-control-prev-icon,
  #content .new-sale .carousel-control-next-icon {
    top: 40%;
  }
}
</style>
    ...
    getNewProducts () {
      ...
      vm.$http.get(api).then(response => {
        vm.newProduct = response.data.products.filter(item => {
          const index = vm.newProductList.findIndex(
            item1 => item1 === item.title
          )
          if (index === -1) {
            return false
          } else {
